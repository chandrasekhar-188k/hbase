...
public class TestCompactionServerBase {
  ...
  public static void beforeClass() throws Exception {
    TEST_UTIL.startMiniCluster(StartTestingClusterOption.builder().numCompactionServers(1).build());
  ...
  protected void doPutRecord(int start, int end, boolean flush) throws Exception {
  ...
  protected void doFillRecord(int start, int end, byte[] value) throws Exception {
    Table h = TEST_UTIL.getConnection().getTable(TABLENAME);
    for (int i = start; i <= end; i++) {
      Put p = new Put(Bytes.toBytes(i));
  ...
  protected void verifyRecord(int start, int end, boolean exist) throws Exception {
    Table h = TEST_UTIL.getConnection().getTable(TABLENAME);
    for (int i = start; i <= end; i++) {
