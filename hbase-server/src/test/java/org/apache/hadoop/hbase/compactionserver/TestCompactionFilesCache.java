...
public class TestCompactionFilesCache {
  ...
  public void testSelectedFilesStoreMultiThread() {
    ...
    for (int i = 0; i < threadNum; i++) {
      testStorageThreads[i] = new TestStorageThread(new ArrayList<>(Arrays.asList("" + i)), true);
    }
    for (int i = 0; i < threadNum; i++) {
    ...
    for (int i = 0; i < threadNum; i++) {
      testStorageThreads[i] = new TestStorageThread(new ArrayList<>(Arrays.asList("" + i)), false);
    }
    for (int i = 0; i < threadNum; i++) {
      testStorageThreads[i].start();
    }
