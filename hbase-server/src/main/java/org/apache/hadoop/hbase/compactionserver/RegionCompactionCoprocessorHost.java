 ...
 * load coprocessor involves compaction. 2.Other methods of the host, like
 * preFlush,postFlush,prePut, postPut, etc will be not supported. 3.Four methods: preOpen, postOpen,
 * preClose, postClose are overridden as blank implementations. 4.The methods preCompactSelection,
 * postCompactSelection, preCompactScannerOpen, preCompact, postCompact, preStoreFileReaderOpen,
 * postStoreFileReaderOpen, postInstantiateDeleteTracker will be retained.
 */
...
public class RegionCompactionCoprocessorHost extends RegionCoprocessorHost {
  ...
  /**
   * Invoked before a region open. For invoke compact, we will open fake region on compaction
   * server. So implement preOpen as blank
   */
  public void preOpen() throws IOException {
  }

  /**
   * Invoked after a region open For invoke compact, we will open fake region on compaction server.
  ...
  /**
   * Invoked before a region is closed For invoke compact, we will open fake region on compaction
   * server. So implement preClose as blank
   */
  public void preClose(final boolean abortRequested) throws IOException {
  }

  /**
   * Invoked after a region is closed For invoke compact, we will open fake region on compaction
   * server. So implement postClose as blank
   */
